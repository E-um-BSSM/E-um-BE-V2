#!/usr/bin/env bash
# 이 스크립트는 로컬(Mac) 터미널에서 실행하세요. gh CLI 인증(gh auth login)이 되어 있어야 합니다.
# 순서대로 이슈 생성 -> 브랜치 push -> PR 생성 -> PR 머지를 반복합니다.
# main과 각 브랜치는 이미 git bundle(eum-be-v2-history.bundle)로 가져온 상태여야 합니다.
...
ISSUE_BODIES=(
"## 작업 내용
Gradle 프로젝트 스캐폴딩에 JWT(jjwt)/메일(spring-boot-starter-mail) 의존성을 추가하고, application.properties에 JWT·메일·JSON snake_case 설정을 구성한다. 테스트는 H2로 돌아가도록 설정한다.

## 세부 작업
- [ ] build.gradle: jjwt, spring-boot-starter-mail, h2(test) 추가
- [ ] application.properties: JWT/메일/snake_case 설정 추가
- [ ] src/test/resources/application.properties: H2 테스트 설정 추가
- [ ] CONTRIBUTING.md: 커밋/브랜치/이슈/PR 컨벤션 문서화

## 참고
- EumMVPApi.json, erd.txt"
